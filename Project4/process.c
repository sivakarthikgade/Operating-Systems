...
{ 
  ...
  copy_first_frame_into_memory(idlePid);
...
char *fname;
{
  ...
  pid = get_PCB ();
  if(mError == allocate_memory (pid, msize, numinstr)) {
	  free_PCB(pid);
	  ...
	  copy_first_frame_into_memory(pid);

	  // put process into ready list
	  if (PCB[pid]->exeStatus == eReady) insert_ready (pid);
