...
int main(int argc, char *argv[])
{
	...
	while( ( fgets(cmd, 100, file_ptr) ) != NULL ) {
     ...
     int y, result;
     while(1) {
    	 while(adder_instr_rcvd_cnt < 550) continue;
		  if(((adder_instr_rcvd_cnt-instr_cmpl_cnt)>=N) && (flag==0)) {
			  ...
			  flag = 0;
			  n = sendto(mainpsockfd,(char *) "rsadd", 5*sizeof(char), 0, (struct sockaddr *) &mainp_serv_addr, mainp_serv_addr_len);
			  if (n < 0)
     ...
     n = sendto(mainpsockfd,(char *) "exit_adder", 10*sizeof(char), 0, (struct sockaddr *) &mainp_serv_addr, mainp_serv_addr_len);
     if (n < 0)
    	 error("Adder: ERROR writing to socket using UDP.\n");
